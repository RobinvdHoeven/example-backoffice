...
@section('content')
    <div class="participants fullwidth">
        <h1 style="margin-top:0;">Goedgekeurd: ({{$orders->total()}})</h1>
        <button class="postbtn" onclick="location.href='{{ route('pending')}}'">In afwachting</button>
        <button class="postbtn" style="color:#f7ea4e;" onclick="location.href='{{ route('confirmed')}}'">Goedgekeurd</button>
        ...
        <div class="search">
            <form method="post" class="searchbar" action="{{ route('orders.search') }}">
                {{ csrf_field() }}
            ...
            <br><br>
            @if(count($orders) != 0)
                @if(\Request::is('bestellingen/zoekenorders'))
                ...
                @else
                    <form class="searchbar" action="{{ route('orders.export') }}">
                        <button class="postbtn" type="submit">Download naar Excel</button>
                    </form>
                @endif
            @endif
